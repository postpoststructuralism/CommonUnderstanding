...
- **Understanding graph** — connects propositions, arguments, evidence, contradictions, syntheses, and snapshots for exploration. A database-derived static skeleton is embedded for an immediate first paint, the browser loads the latest versioned skeleton and incrementally enriches visible regions from live graph endpoints as the user zooms, and a freshness-driven hosted worker regenerates stale skeleton files after user activity becomes quiet without running AI analysis.
